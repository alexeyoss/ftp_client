...
internal class ConnectionAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.title.setOnClickListener {
            val options = Bundle()
            options.putInt(KEY_CONNECTION_ID, connections[position].id)
            try {
        ...
        holder.title.setOnLongClickListener {
            ConnectionActionsBottomSheet.newInstance(connections[position].id)
                .show(fm, ConnectionActionsBottomSheet::javaClass.name)
            true
        }
    }
